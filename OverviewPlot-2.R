  OverviewPlot <- function(school) {
    ...
    ### Plots
    ## Profile
    o.daily <- ddply(o,c("Date"), summarise, AvgOutdoorTemp=mean(OutdoorTemp, na.rm=TRUE), AvgOutdoorUTCI=mean(OutdoorUTCI, na.rm=TRUE)) #aggregated by date 
    cr.daily <- ddply(cr, c("Date", "Alias"), summarise, AvgRoomTemp=mean(Temp, na.rm=TRUE), MaxRoomTemp=max(Temp,na.rm=TRUE), AvgRoomUTCI=mean(UTCI, na.rm=TRUE), MaxRoomUTCI=max(UTCI, na.rm=TRUE))
    ...
    p <- ggplot() +
      ...
      geom_smooth(data=daily.melt[daily.melt$Alias %in% c(school, "Outdoor"),], aes(x=Date, y=value, linetype=variable, size=variable, color=variable), fill=NA) +
    ...
    models <- ldply(dlply(d3.daily, c("Alias", "variable"), function(x) {loess(value ~ as.numeric(Date), data=x, model=TRUE)$fitted}), data.frame)
    d3.daily$smooth <- models$X..i..

    write.csv(d3.daily, file="~/dropbox/rh1/hidoe/final-csv/daily.csv", row.names=FALSE, na="")
    ...
    write.csv(d3.bar , file="~/dropbox/rh1/hidoe/final-csv/bar.csv", row.names=FALSE, na="")
    ...
    o.hourly$variable <- ifelse(o.hourly$variable=="OutdoorTemp", "AvgOutdoorTemp", "AvgOutdoorUTCI")
    ...
    l3 <- ggplot() + 
      geom_hline(yintercept=minF, linetype="dotted", color="black", size=0.5) +
      geom_line(data=cr.hourly[cr.hourly$School==school,], aes(x=Time, y=value, group=Alias, color=Alias), size=0.5, alpha=0.5, show.legend = FALSE) +
      geom_line(data=o.hourly, aes(x=Time, y=value, linetype=variable, size=variable), color="dimgrey") +
      facet_grid(~monthdisp, drop=FALSE) +
      scale_y_continuous(breaks=seq(70,100,5), limits=c(70,100)) +
      scale_color_manual(values=palette(ann.size)) +
    ...
    write.csv(d3.line , file="~/dropbox/rh1/hidoe/final-csv/line.csv", row.names=FALSE, na="")
